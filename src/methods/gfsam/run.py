"""GF-SAM CLI. ``python -m src.methods.gfsam``."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    add_eval_args(parser, default_output="results/gfsam", default_image_size=1024)
    ...
    def predict(ep: Episode) -> np.ndarray:
        ...
        if any(mask.sum().item() == 0 for mask in masks):
            warnings.warn(
                ...
                "GF-SAM will punch a 14×14 centre square",
                stacklevel=2,
            )
        ...
        try:
            ...
            pred_masks, _ = model.predict()
        finally:
            model.clear()
        pred = pred_masks.detach().cpu().numpy()
        pred = np.squeeze(pred)
        if pred.ndim != 2:
            raise ValueError(f"GF-SAM pred has shape {pred.shape}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    from src.methods.gfsam.build import build_gfsam

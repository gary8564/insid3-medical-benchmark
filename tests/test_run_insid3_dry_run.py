...
def test_dry_run_builds_episodes_without_loading_insid3(tmp_path: Path):
    ...
    assert all(row["reference_id"] != row["target_id"] for row in payload)
    assert {row["reference_id"] for row in payload} != {"ref"}
    assert payload[0]["dataset"] == "polyp"
    assert payload[0]["episode_index"] == 0
    for row in payload:
        assert Path(row["reference_image"]).is_file()
        assert Path(row["reference_mask"]).is_file()
